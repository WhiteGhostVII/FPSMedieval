...
public class RigidbodyWalk : MonoBehaviour
{
    ...
    {        
        //StartCoroutine(Idle());
        //referenceObject = Camera.main.GetComponent<PlayerShootRigidbody>().GetReferenceObject();
    }
    ...
    void FixedUpdate()
    {
        ...
        rdb.AddForce(globalmove * (movforce/(rdb.velocity.magnitude + 1)));
    }
    //IEnumerator Idle()
    //{
    //    state = States.idle;
